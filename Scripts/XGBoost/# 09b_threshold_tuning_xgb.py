# 09b_threshold_tuning_xgb_scaled.py
import os, json
import numpy as np
import pandas as pd
import joblib
...
RES_THR_DIR = r"C:\Users\49176\Desktop\Studium\Masterarbeit_3\Results\XGB\Thresholds"
FINAL_DIR   = r"C:\Users\49176\Desktop\Studium\Masterarbeit_3\Results\XGB"
os.makedirs(RES_THR_DIR, exist_ok=True)
os.makedirs(FINAL_DIR, exist_ok=True)

# Business values
V_TP = 30   # Value per True Positive
...
def choose_threshold_profit(y_true, y_prob, V_TP=30, C_FP=-5, plot=False, variant="xgb"):
...
for name, (Xfile, yfile) in datasets.items():
    print(f"[INFO] Threshold tuning (validation) for XGB — {name.upper()}")

    model = joblib.load(os.path.join(MODEL_DIR, f"xgb_tuned_{name}.pkl"))
    ...
    out_model_dir = os.path.join(MODEL_DIR, f"final_{name}")
    os.makedirs(out_model_dir, exist_ok=True)
    joblib.dump(model, os.path.join(out_model_dir, "model_final.pkl"))
    joblib.dump(scaler, os.path.join(out_model_dir, "scaler.pkl"))
    with open(os.path.join(out_model_dir, "threshold_used.json"), "w", encoding="utf-8") as f:
    ...
    pd.DataFrame([{"dataset": name, **metrics}]).to_csv(
        os.path.join(FINAL_DIR, f"xgb_metrics_{name}.csv"), index=False
    )

    # Save precision-recall sweep
    p, r, thr = precision_recall_curve(y_val, val_prob)
    sweep_df = pd.DataFrame({"precision": p[:-1], "recall": r[:-1], "threshold": thr})
    sweep_df.to_csv(os.path.join(RES_THR_DIR, f"xgb_val_pr_sweep_{name}.csv"), index=False)

    rows.append({"model": "XGB", "variant": name, **metrics})

print(" XGB thresholds tuned using business profit. Outputs saved.")
